...
const CitizenDashboard = () => {
    ...
    const getStageColor = (stage) => {
        switch(stage) {
            ...
            case 'Approved': return 'var(--primary-hover)'; 
    ...
    if (!stats) return (
        <div style={{ backgroundColor: 'var(--bg-main)', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'var(--primary)' }}>
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '20px', marginBottom: '40px' }}>
                    <div>
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: '1rem', margin: 0 }}>Civic Dashboard Overview</p>
                    </div>
                    ...
                    {[
                        ...
                        <button
                            ...
                            style={{
                                ...
                                border: 'none',
                                padding: '12px 24px',
                                color: activeTab === tab.id ? '#74B49B' : 'var(--text-muted)',
                    ...
                    <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px', marginBottom: '48px' }}>
                            {[
                                ...
                                { icon: 'fa-bullhorn', color: 'var(--primary-hover)', bg: 'rgba(74, 156, 124, 0.15)', value: myReports.length, label: 'Issues Reported' },
                                { icon: 'fa-medal', color: 'var(--primary)', bg: 'rgba(37, 99, 235, 0.15)', value: stats.badges?.length || 0, label: 'Badges Earned' }
                            ].map((stat, i) => (
                                <div key={i} style={{
                        ...
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                                ...
                                <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.5rem', margin: 0, color: 'var(--text-main)' }}>Community Verification Queue</h3>
                            </div>
                            <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', marginBottom: '24px', marginLeft: '52px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '16px' }}>
                                {pendingVerifications.length === 0 ? (
                                    <div style={{ gridColumn: '1 / -1', background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '16px', padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
                                        ...
                                        return (
                                            <div key={issue.id} style={{
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                                                    <h4 style={{ margin: 0, fontFamily: "'Outfit', sans-serif", fontSize: '1.1rem', color: 'var(--text-main)' }}>{issue.title}</h4>
                                                ...
                                                <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: '0 0 20px 0' }}><i className="fa-solid fa-location-dot"></i> {issue.location}</p>
                                                ...
                                                {iVerified ? (
                                                    ...
                                                    <button onClick={() => handleVerify(issue.id)} style={{
                                                        width: '100%', padding: '12px', background: 'linear-gradient(135deg, var(--primary), var(--primary-hover))', border: 'none',
                                                        borderRadius: '12px', color: 'var(--text-main)', fontSize: '0.9rem', fontWeight: '600', cursor: 'pointer',
                    ...
                    <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.5rem', margin: 0, color: 'var(--text-main)' }}>My Reported Issues</h3>
                            </div>
                            
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <label style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Filter Status:</label>
                                <select 
                                    value={statusFilter} 
                                ...
                                >
                                    {uniqueStatuses.map(s => (
                                        <option key={s} value={s} style={{ background: 'var(--bg-main)', color: 'var(--text-main)' }}>{s}</option>
                                    ))}
                                </select>
                            </div>
                        </div>
                                    ...
                                    <div key={report.id} style={{
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                                                <h4 style={{ margin: 0, fontFamily: "'Outfit', sans-serif", fontSize: '1.1rem', color: 'var(--text-main)' }}>{report.title}</h4>
                                                <span style={{ 
                                            ...
                                            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: '0 0 12px 0' }}>{new Date(report.date).toLocaleDateString()} &bull; {report.location}</p>
                                            
                                            <div>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: '4px', color: 'var(--text-main)' }}>
                ...
                {activeTab === 'profile' && (
                    <div style={{ maxWidth: '600px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px' }}>
                            <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(37, 99, 235, 0.1)', color: 'var(--primary-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}>
                                <i className="fa-solid fa-address-card"></i>
                            </div>
                            <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.5rem', margin: 0, color: 'var(--text-main)' }}>Account Settings</h3>
                        </div>
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.02)', padding: '32px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <div style={{ display: 'flex', gap: '24px', alignItems: 'center', marginBottom: '32px', paddingBottom: '32px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                                <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary), var(--primary-hover))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.5rem', color: 'var(--text-main)', fontWeight: 'bold' }}>
                                    {stats.username.charAt(0).toUpperCase()}
                                </div>
                                <div>
                                    <h3 style={{ margin: '0 0 8px 0', fontSize: '1.5rem', fontFamily: "'Outfit', sans-serif", color: 'var(--text-main)' }}>{stats.fullname || stats.username}</h3>
                            ...
                            <button style={{
                                ...
                                borderRadius: '12px', color: 'var(--text-main)', fontSize: '1rem', fontWeight: '600',
